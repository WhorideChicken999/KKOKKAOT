...
class FashionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return {
            ...
            'length': item['length_encoded']
        }

class BottomDressFashionModel(nn.Module):
    def __init__(self, num_category, num_color, num_material, num_print, num_fit, num_style, num_length):
        super(BottomDressFashionModel, self).__init__()
...
def load_schema(schema_path):
    ...
    # 기장 (하의/원피스)
    schema['length'] = df[df['subcategory'] == 'Length']['label_ko'].tolist()
    
    return schema

def match_label_image_pairs(base_path, target_categories):
    """라벨과 이미지 파일을 매칭"""
    cnn_labels_path = os.path.join(base_path, 'cnn_labels')
    cropped_images_path = os.path.join(base_path, 'cropped_images')
    
    print(f"\n=== 라벨-이미지 매칭 시작 ({', '.join(target_categories)}) ===")
...
def prepare_data(base_path, schema, target_categories, sample_size=None):
    """데이터 준비 및 인코딩"""
    # 라벨-이미지 매칭
    matched_pairs = match_label_image_pairs(base_path, target_categories)
    ...
    # 라벨 인코딩 준비 (모두 단일 레이블)
    encoders = {}
    for attr in ['category', 'color', 'material', 'print', 'fit', 'style', 'length']:
...
def train_model(model, train_loader, val_loader, device, encoders, schema, output_dir, num_epochs=1):
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, batch in train_bar:
            ...
            # 라벨
            labels = {attr: batch[attr].to(device) for attr in ['category', 'color', 'material', 'print', 'fit', 'style', 'length']}
...
def main():
    ...
    output_dir = r'D:\kkokkaot\models\bottom_dress'
    target_categories = ['하의', '원피스']
    ...
    data_list, encoders = prepare_data(base_path, schema, target_categories, sample_size=sample_size)
