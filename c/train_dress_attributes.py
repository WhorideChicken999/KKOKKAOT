...
def load_schema(schema_path):
    ...
    # 카테고리 (원피스만)
    schema['category'] = df[(df['category'] == 'Item') & (df['subcategory'] == 'Dress')]['label_ko'].tolist()
...
def prepare_data(base_path, schema, target_category, sample_size=None):
    ...
    # 라벨 인코딩 준비 (모두 단일 레이블)
    encoders = {}
    for attr in ['category', 'color', 'material', 'print', 'style']:
...
def train_model(model, train_loader, val_loader, device, encoders, schema, output_dir, num_epochs=1):
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, batch in train_bar:
            ...
            # 라벨
            labels = {attr: batch[attr].to(device) for attr in ['category', 'color', 'material', 'print', 'style']}
